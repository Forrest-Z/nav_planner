...
public:
  ...
  void update(const mpac_msgs::Task &msg) {
  ...
  void update(const mpac_msgs::ForkReportConstPtr &msg, bool &completedTarget, bool &moveForks, bool &load, OperationState &operation) {
  ...
  void update(const mpac_msgs::ControllerReportConstPtr &msg, bool &completedTarget, bool &recomputeNewTrajectory, bool useForks = false) {
    ...
    mpac_generic::State2d state = mpac_conversions::createState2dFromControllerStateMsg(msg->state);
    ...
    if (!trajectoryChunks_.empty()) {
      ...
      currentTrajectoryChunkEstIdx_ = mpac_generic::getCurrentChunkIdx(trajectoryChunks_, state.getPose2d(), currentTrajectoryChunkEstIdx_);
    ...
    if (path_.sizePath() > 0) {
      mpac_generic::State2d state = mpac_conversions::createState2dFromControllerStateMsg(msg->state);
      currentPathIdx_ = mpac_generic::getCurrentIdx(path_, state.getPose2d(), currentPathIdx_);
    }
  ...
  mpac_generic::TrajectoryChunks getTrajectoryChunks() const {
  ...
  const mpac_generic::TrajectoryChunks& getTrajectoryChunksRef() const {
    return trajectoryChunks_;
  }
  
  void appendTrajectoryChunks(unsigned int chunkIdx, const mpac_generic::TrajectoryChunks &add) {
    this->trajectoryChunks_ = mpac_generic::appendChunks(this->getTrajectoryChunks(), chunkIdx, add);
  }
  ...
  //  void setTrajectoryChunks(const mpac_generic::TrajectoryChunks &trajChunks) {
  ...
  bool canSendActivate() const {
    const mpac_msgs::ControllerReport msg;
    if (controller_status_ == msg.CONTROLLER_STATUS_WAIT)
      return true;
    return false;
  }
  ...
  void setDockingPose(const mpac_generic::Pose2d &pose) {
    dockingPose_ = pose;
  }

  mpac_generic::Pose2d getDockingPose() const {
    return dockingPose_;
  }
  ...
  void setPath(const mpac_generic::Path &path) {
    path_ = path;
  }

  void setCoordinatedTimes(const mpac_generic::CoordinatedTimes &cts) {
    cts_ = cts;
  }

  bool updatePath(const mpac_generic::Path &path) {
    if (path_.sizePath() == 0) {
      path_ = path;
      return true;
    }
    mpac_generic::Pose2d current_start = path_.getPose2d(0);
    mpac_generic::Pose2d current_goal = path_.getPose2d(path_.sizePath()-1);
    double dist1 = mpac_generic::getDistBetween(current_start, path.getPose2d(0));
    double dist2 = mpac_generic::getDistBetween(current_goal, path.getPose2d(0));
    ...
    mpac_generic::addPathToPath(path_, path);
    mpac_generic::makeValidPathForTrajectoryProcessing(path_);
  }
  
  //! Return the path, if the criticalIdx is set the path up to this index will be returned.
  mpac_generic::Path getPath() const {
    if (this->getCriticalPointIdx() >= 0) {
      if (this->getCriticalPointIdx()+1 < path_.sizePath()) {
        return mpac_generic::selectPathIntervall(path_, 0, this->getCriticalPointIdx()+1);
      }
      else {
  ...
  mpac_generic::Path getPathFromLastAssignedTask() const { 
    return mpac_conversions::createPathFromPathMsgUsingTargetsAsFirstLast(task_.path);
  }

  //! Return the cts, if the criticalIdx is set the path up to this index will be returned.
  mpac_generic::CoordinatedTimes getCoordinatedTimes() const {
    if (this->getCriticalPointIdx() >= 0) {
      if (this->getCriticalPointIdx()+1 < cts_.size()) {
        return mpac_generic::selectCtsInterval(cts_, 0, this->getCriticalPointIdx()+1);
      }
      else {
  ...
  mpac_generic::State2d getCurrentState2d() const {
  ...
  mpac_generic::Control getCurrentControl() const {
  ...
  int getEarliestPathIdxToConnect() {
    if (this->getCurrentTrajectoryChunkIdx() < 0)
      return -1;
    mpac_generic::TrajectoryChunks chunks = this->getTrajectoryChunks();
    unsigned int chunk_idx = this->getCurrentTrajectoryChunkIdx()+3; // Approx. 1.5 sec ahead.
    if (chunk_idx >= chunks.size())
      return -1;
    mpac_generic::Path path = this->getPath();
    double distance;
    int idx = getPathIdxUsingFirstPointInChunk3(path, chunks, chunk_idx, distance);
    return idx;
  }

  mpac_generic::Trajectory getDrivenTrajectory() const { return drivenTrajectory_; }
  mpac_generic::CoordinatedTimes getDrivenTrajectoryTimes() const { return drivenTrajectoryTimes_; }

  mpac_msgs::Task getTask() const { return task_; }

  mpac_generic::State2d getCurrentGoalState() const {
    if (path_.sizePath() == 0) {
      return currentState2d_;
    }
    return mpac_generic::State2d(path_, path_.sizePath()-1);
  }

  void saveCurrentTrajectoryChunks(const std::string &fileName) const {
    mpac_generic::saveTrajectoryChunksTextFile(trajectoryChunks_, fileName);
  }

  mpac_generic::RobotInternalState2d getInternalState2d() const {
    return internalState2d_;
  }
  
  mpac_msgs::RobotReport getReport() const {
    mpac_msgs::RobotReport msg;
    msg.stamp = currentTime_;
    msg.status = state_;
    msg.state = mpac_conversions::createPoseSteeringMsgFromState2d(currentState2d_);
    msg.sequence_num = currentPathIdx_;
    return msg;
  }

  mpac_geometry::PalletModel2dWithState getPalletModelLoadDetect() const {
...
private:
  ...
  mpac_generic::TrajectoryChunks trajectoryChunks_;
  ...
  mpac_generic::Path path_;
  mpac_generic::CoordinatedTimes cts_;
  unsigned int currentPathIdx_;
  double trajectoryChunksStartTime_;
  bool dockingFailed_;
  mpac_generic::Pose2d dockingPose_;
  mpac_generic::State2d currentState2d_; // Current state from the controller report.
  mpac_generic::Control currentControl_;
  ...
  mpac_generic::Trajectory drivenTrajectory_;
  mpac_generic::CoordinatedTimes drivenTrajectoryTimes_; // Global time when each trajectory_ point was reached.

  mpac_msgs::Task task_; // Last task requested (by the update function)
  mpac_msgs::Task activeTask_; // The task that is currently executed.

  mpac_generic::RobotInternalState2d internalState2d_;
