...
std::pair<unsigned int, mpac_generic::TrajectoryChunks> computeTrajectoryChunksCASE3(const VehicleState &vs, const TrajectoryProcessor::Params &params, bool useCts) {
  
  mpac_generic::Path path = vs.getPath();
  
  TrajectoryProcessorNaiveCT gen;
  gen.addPathInterface(path);
  if (useCts) {
    mpac_generic::CoordinatedTimes cts = vs.getCoordinatedTimes();
    gen.addCoordinatedTimes(cts);
  }
  gen.setParams(params);
  mpac_generic::Trajectory traj = gen.getTrajectory();
  gen.printDebug();
  std::pair<unsigned int, mpac_generic::TrajectoryChunks> ret;
  ret.first = 0;
...
std::pair<unsigned int, mpac_generic::TrajectoryChunks> computeTrajectoryChunksCASE1(VehicleState &vs, const TrajectoryProcessor::Params &params, unsigned int &pathIdx, bool useCts) {
  ...
  {
    mpac_generic::State2d state2d = vs.getCurrentState2d();
    std::vector<std::pair<size_t, double> > res = mpac_generic::findClosestStatesVec(path, state2d); 
    ...
    if (useCts) {
      mpac_generic::CoordinatedTimes cts = mpac_generic::truncateCts(vs.getCoordinatedTimes(), res[iter].first);
...
// Vehicle is driving.
std::pair<unsigned int, mpac_generic::TrajectoryChunks> computeTrajectoryChunksCASE2(const VehicleState &vs, const TrajectoryProcessor::Params &params, const unsigned int &chunkIdx, unsigned int &pathIdx, double &pathChunkDistance, bool &valid, bool useCts) {
  ...
  const mpac_generic::TrajectoryChunks& current_chunks = vs.getTrajectoryChunksRef();
  ...
  // 4), recompute the trajectory.
  {
    mpac_generic::State2d state2d(current_chunks.getChunk(chunkIdx), 0);
    std::vector<std::pair<size_t, double> > res = mpac_generic::findClosestStatesVec(path, state2d); 
    // Check that the path is in the right direction (maybe the second closest is better).
    assert(res.size() > 2);
    ...
    if ((pathIdx+3)> path.sizePath()) {
      ...
      return std::pair<unsigned int, mpac_generic::TrajectoryChunks>();
    }
    ...
    mpac_generic::makeValidPathCoordinatedTimesForTrajectoryProcessing(path,cts);
...
double getTargetStartGoalDistance(const mpac_msgs::RobotTarget &target) {
  mpac_generic::State2d start = mpac_conversions::createState2dFromPoseSteeringMsg(target.start);
  mpac_generic::State2d goal = mpac_conversions::createState2dFromPoseSteeringMsg(target.goal);
  return mpac_generic::getDistBetween(start.getPose2d(), goal.getPose2d());
}

bool getRepositioningPathUsingDrivenPath(mpac_generic::Path &path, const mpac_generic::PathInterface &drivenPath, double repositioningDistance) {
  
  if (drivenPath.sizePath() == 0)
    return false;
  
  double used_dist;
  path = mpac_generic::getRepositioningPath(drivenPath, repositioningDistance, used_dist);
...
bool getRepositioningPathMsgUsingDrivenPath(mpac_msgs::Path &path, const mpac_msgs::RobotTarget &target, const mpac_generic::PathInterface &drivenPath, double repositioningDistance) {
  mpac_generic::Path p;
  if (!getRepositioningPathUsingDrivenPath(p, drivenPath, repositioningDistance)) {
    return false;
  }
  path = mpac_conversions::createPathMsgFromPathInterface(p);
