...
bool getPolygonConstraintsCallback(mpac_msgs::GetPolygonConstraints::Request  &req,
         mpac_msgs::GetPolygonConstraints::Response &res )
{
...
//  mpac_generic::Path path = mpac_conversions::createPathFromPathMsgUsingTargetsAsFirstLast(req.path);
  ...
  mpac_generic::Pose2d target_start_pose = mpac_conversions::createPose2dFromMsg(req.path.target_start.pose);
  mpac_generic::Pose2d target_goal_pose = mpac_conversions::createPose2dFromMsg(req.path.target_goal.pose);
...
//          found = lookup->findConstraint(req.map, path.getPose2d(i), constraint, constraint_idx);
      ...
      mpac_rviz::drawPose2d(path.getPose2d(i), i, 2, 0.5, "current_pose", marker_pub);
      if (found) {
          ...
          mpac_geometry::Polygon outer_constraint = lookup->computeConstraintOuterRegion(constraint);
          ...
          if (visualize) {
              ...
              //mpac_rviz::drawPoint2dContainerAsConnectedLine(constraint.getInnerConstraint(), "global_best_constraint", i, 1, marker_pub);
              mpac_rviz::drawPose2d(mpac_generic::Pose2d(pose(0), pose(1), constraint.getThBounds()(0)), i, 2, 0.5, "global_th_left", marker_pub);
              mpac_rviz::drawPose2d(mpac_generic::Pose2d(pose(0), pose(1), constraint.getThBounds()(1)), i, 2, 0.5, "global_th_right", marker_pub);
              if (visualize_outer_constraints) {
                //mpac_geometry::Polygon outer_constraint = lookup->computeConstraintOuterRegion(constraint);
                  mpac_rviz::drawPoint2dContainerAsConnectedLine(outer_constraint, "global_best_outer_constraint", i, 0, marker_pub);
              }
          }
      }
      else {
  ...
  if (!invalid_idx.empty()) {
      ...
      for (size_t i = 0; i < invalid_idx.size(); i++) {
          ...
          if (visualize) {
              mpac_rviz::drawPoint2dContainerAsConnectedLine(constraints[invalid_idx[i]].getInnerConstraint(), "invalid_path_overlap", invalid_idx[i], 1, marker_pub);
              mpac_rviz::drawPoint2dContainerAsConnectedLine(constraints[invalid_idx[i]+1].getInnerConstraint(), "invalid_path_overlap", invalid_idx[i]+1, 1, marker_pub);
          }
      }
  }
  else {
  ...
  if (save_constraints_and_path) {
      // Save the path (standard text file).
      {
          ...
          mpac_generic::savePathTextFile(path, fn);
      }
      ...
      // Save the outer constraints (as a text file)
      {
          ...
          for (size_t i = 0; i < constraints.size(); i++) {
              mpac_geometry::Polygon outer_constraint = lookup->computeConstraintOuterRegion(constraints[i]);
              ofs << mpac_generic::getPoint2dContainerInterfaceGnuplotString(outer_constraint, true);
              ofs << "\n";
          }
          ofs.close();
      }
  ...
  if (visualize) {
      ...
      mpac_rviz::drawConstraints(res.constraints, marker_pub);
  }
...
int main(int argc, char **argv)
{
  ...
  mpac_geometry::RobotModel2dInterface* model = model_type_factory.getModel(model_type);
  ...
  switch (load_type) {
  case 1:
      s.loadType = mpac_generic::RobotInternalState2d::NO_LOAD;
      break;
  case 2:
      s.loadType = mpac_generic::RobotInternalState2d::EUR_PALLET;
      break;
  case 3:
      s.loadType = mpac_generic::RobotInternalState2d::HALF_PALLET;
      break;
  };
