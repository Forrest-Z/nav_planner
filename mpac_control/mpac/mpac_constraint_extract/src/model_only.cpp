#include <mpac_constraint_extract/model_only.h>

using namespace constraint_extract;

ConstraintExtractorModelOnly::ConstraintExtractorModelOnly(const mpac_generic::PathInterface &p, const mpac_geometry::RobotModel2dInterface &m, mpac_generic::RobotInternalState2d::LoadType loadType) : path(p), model(m), computed_(false), loadType_(loadType) {
...
ConstraintExtractorModelOnly::compute() {
  ...
  // Subsample the path
  mpac_generic::SubsamplePath sp(path, params.minDist, params.maxDist, params.maxRotation);
  ...
  if (params.forceParkingPolygons) {
    ...
    for (size_t j = sub_idx[1]; j < sub_idx[2]; j++) {
        double dist = mpac_generic::getDistBetween(path.getPose2d(j), path.getPose2d(0));
    ...
    for (size_t j = sub_idx[sub_idx.size()-2]; j > sub_idx[sub_idx.size()-3]; j--) {
        double dist = mpac_generic::getDistBetween(path.getPose2d(j), path.getPose2d(sub_idx.back()));
        if (dist > max_parking_poly_dist) {
...
mpac_geometry::Polygons 
ConstraintExtractorModelOnly::upSample(const mpac_geometry::Polygons &polygons, const std::vector<size_t>& subsample_idx) {
  assert(!subsample_idx.empty());
  mpac_geometry::Polygons new_polygons;
