...
public:
  ...
  {
    poly_ = mpac_geometry::getPolygonFromBox(topLeft, bottomRight);
    this->computeWeight();
  }

  PolygonConstraint(mpac_generic::Point2dContainerInterface &pts, const Eigen::Vector2d &thBounds) : thBounds_(thBounds)
  {
    poly_ = mpac_geometry::Polygon(pts);
  }
  ...
  const mpac_geometry::Polygon &getInnerConstraint() const { return poly_; }

  const mpac_geometry::Polygon &getOuterConstraint() const { return outerPoly_; }
  void setOuterConstraint(const mpac_geometry::Polygon &poly)
  {
    outerPoly_ = poly;
  }
  ...
  bool findInnerConstraintMatch(const PolygonConstraint &pc) const
  {
    ...
    {
      if (mpac_geometry::distSqrPoint2d(this->poly_.getPoint2d(i), pc.poly_.getPoint2d(i)) > 0.01)
        return false;
    }
  ...
  void moveConstraint(const mpac_generic::Pose2d &pose)
  {
    mpac_geometry::movePoint2dContainer(this->poly_.points, pose);
  ...
  /*!
     ...
     */
  double angularConstraintDistance(const mpac_generic::Pose2d &pose) const
  {
  ...
  /*!
     ...
     * if < small positive value, the constraint is active.
     */
  double positionConstraintDistance(const mpac_generic::Pose2d &pose) const
  {
  ...
  bool isActive(const mpac_generic::Pose2d &pose, double thresh = 0.0001) const
  {
  ...
  bool isFeasible(const mpac_generic::Pose2d &pose) const
  {
  ...
  mpac_geometry::Polygon poly_;
  mpac_geometry::Polygon outerPoly_;
  Eigen::Vector2d thBounds_;

  double weight_;
  mpac_generic::Pose2d feasiblePose_;
...
private:
  void computeWeight()
  {
    ...
    //Eigen::Vector2d pt = mpac_generic::getClosestPoint2dToOrigin(poly_);
    //weight_ = std::min(pt(0), pt(1)) * poly_.getArea() * getThBoundWidth();       //      mpac_generic::getClosestPoint2dToOrigin(poly_).norm()
...
inline mpac_geometry::Polygon computeThSweepPolygon_(const Eigen::Vector2d &point, const Eigen::Vector2d &thBounds, mpac_geometry::RobotModel2dWithState &rm, const mpac_generic::RobotInternalState2d &s, double thIncr)
{
  mpac_geometry::Polygon p;
  if (thBounds(1) - thBounds(0) < thIncr)
  {
    thIncr = thBounds(1) - thBounds(0);
  }

  mpac_generic::Pose2d pose(point(0),
                              point(1),
                              thBounds(0));
  ...
  if (fabs(pose(2) - thBounds(1) - thIncr) > 0.0001)
  { // Check that we're not approx. adding the same polygon again - this will cause some weird behaviour if this difference is very small, (10^-16)
    pose(2) = thBounds(1);
    rm.update(pose, s);
    p.addPolygon(rm.getPosePolygon());
  }
...
inline mpac_geometry::Polygon computeConstraintOuterRegion_(const PolygonConstraint &constraint, mpac_geometry::RobotModel2dWithState &rm, const mpac_generic::RobotInternalState2d &s, double th_sweep_incr)
{
  mpac_geometry::Polygon p;
  const mpac_geometry::Polygon &ic = constraint.getInnerConstraint();
  const Eigen::Vector2d &thb = constraint.getThBounds();

  for (size_t i = 0; i < ic.sizePoint2d(); i++)
  {
    mpac_geometry::Polygon sweep = computeThSweepPolygon_(ic.getPoint2d(i), thb, rm, s, th_sweep_incr);
...
class PolygonConstraintsVec : public std::vector<PolygonConstraint, Eigen::aligned_allocator<PolygonConstraint> >, mpac_generic::Pose2dContainerInterface
{
public:
  virtual mpac_generic::Pose2d getPose2d(size_t idx) const
  {
    return (*this)[idx].feasiblePose_;
  }

  virtual void setPose2d(const mpac_generic::Pose2d &pose, size_t idx)
  {
...
public:
  ...
  PolygonConstraintsLookup(const mpac_geometry::RobotModel2dInterface &m, const mpac_generic::RobotInternalState2d::LoadType &loadType, const PolygonConstraintsLookup::Params &params) : params_(params), model(m), loadType_(loadType), computed_(false) {}

  inline void addPose2dThSweep(mpac_generic::Pose2dVec &poses, double x, double y, double min_th, double max_th, double th_sweep_incr) const
  {
    double th = min_th;
    while (th < max_th)
    {
      poses.push_back(mpac_generic::Pose2d(x, y, th));
      th += th_sweep_incr;
    }
    poses.push_back(mpac_generic::Pose2d(x, y, max_th));
  }

  /*  inline mpac_geometry::Polygon computeThSweepPolygon(const Eigen::Vector2d &point, const Eigen::Vector2d &thBounds, mpac_geometry::RobotModel2dWithState &rm, const mpac_generic::RobotInternalState2d &s, double thIncr) const { */
  /*   mpac_geometry::Polygon p; */
  /*   if (thBounds(1) - thBounds(0) < thIncr) { */
  /* 	thIncr = thBounds(1) - thBounds(0); */
  /*   }  */
  ...
  mpac_geometry::Polygon computeConstraintOuterRegion(const PolygonConstraint &constraint, mpac_geometry::RobotModel2dWithState &rm) const
  {
    mpac_generic::RobotInternalState2d s;
  ...
  mpac_geometry::Polygon computeConstraintOuterRegion(const PolygonConstraint &constraint) const
  {
    mpac_geometry::RobotModel2dWithState rm(model);
    return this->computeConstraintOuterRegion(constraint, rm);
  }

  mpac_geometry::Polygon computeLargestConstraintOuterRegion() const
  {
    mpac_geometry::RobotModel2dWithState rm(model);
    mpac_generic::RobotInternalState2d s;
    s.loadType = loadType_;
  ...
  nav_msgs::OccupancyGrid createOccupancyGridPatch()
  {
    ...
    ret.info.origin = mpac_conversions::createMsgFromPose2d(mpac_generic::Pose2d(-3.0, -3.0, 0.));
  ...
  void compute()
  {
    ...
    { // Add the biggest fotprint (the vehicle will never be outside these bound).
      ...
      mpac_geometry::Polygon p = this->computeLargestConstraintOuterRegion();
    ...
    for (size_t i = 0; i < this->constraints_.size(); i++)
    {
      ...
      mpac_geometry::Polygon p = this->computeConstraintOuterRegion(constraints_[i], rm);
  ...
  /* mpac_generic::Point2dVec getOccupiedPixelsInLocalMetricCoords(const nav_msgs::OccupancyGrid &map, const mpac_generic::Pose2d &pose, const nav_msgs::OccupancyGrid &localMap) const { */
  ...
  /*   mpac_generic::Point2dVec pts = getPoint2dVecFromGridPatchIdx(localMap, occ_patch_local); */
  /*   mpac_geometry::movePoint2dContainer(pts, pose); */
  ...
  /*   mpac_generic::Pose2d inv_pose = mpac_generic::subPose2d(pose, mpac_generic::Pose2d(0.,0.,0.)); */
  /*   mpac_geometry::movePoint2dContainer(pts, inv_pose); */
  /*   return pts; */
  /* } */

  // Return the corresponding pixel which is occupied in the map using pixel coords in the local map.
  GridPatchIdx getOccupiedPixelsInLocalPixelCoords(const nav_msgs::OccupancyGrid &map, const mpac_generic::Pose2d &pose, const nav_msgs::OccupancyGrid &localMap) const
  {
  ...
  nav_msgs::OccupancyGrid createLocalOccupancyGridPatchFromPose(const nav_msgs::OccupancyGrid &map, const mpac_generic::Pose2d &pose) const
  {
  ...
  bool findConstraint(const nav_msgs::OccupancyGrid &map, const mpac_generic::Pose2d &pose, PolygonConstraint &result, int &resultIdx) const
  {
  ...
  /*!
     ...
     */
  bool findConstraint(const nav_msgs::OccupancyGrid &map, const mpac_generic::Pose2d &pose, const mpac_generic::Pose2d &containPose, PolygonConstraint &result, int &resultIdx) const
  {
    ...
    if (!params_.skip_overlap)
    {
      ...
      mpac_generic::Pose2d contain_pose_relative = mpac_generic::subPose2d(pose, containPose);
      std::map<int, PolygonConstraint &>::iterator it = valid.begin();
      while (it != valid.end())
      {
