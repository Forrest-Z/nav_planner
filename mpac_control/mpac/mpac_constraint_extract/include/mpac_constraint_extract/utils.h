...
namespace constraint_extract {
  ...
  inline bool validConstraintWithPose(const constraint_extract::PolygonConstraint &pc, const mpac_generic::Pose2d &pose) {
#if 0
  ...
  inline bool validConstraintPathOverlap(const constraint_extract::PolygonConstraint &pc1, const mpac_generic::Pose2d &pose1,
					 const constraint_extract::PolygonConstraint &pc2, const mpac_generic::Pose2d &pose2) {
  ...
  std::vector<int> getInvalidConstraintPathOverlap(const constraint_extract::PolygonConstraintsVec &pcs, const mpac_generic::PathInterface &path) {
  ...
  void printConstraintDistances(const constraint_extract::PolygonConstraintsVec &pcs, const mpac_generic::Pose2dContainerInterface &poses) {
  ...
  PolygonConstraint getMaximumDistanceConstraintToPose(const constraint_extract::PolygonConstraintsVec &pcs, const mpac_generic::Pose2d &pose) {
    assert(pcs.size() > 0);
  ...
  PolygonConstraintsVec reassignNewConstraintsToPose2dInterface(const constraint_extract::PolygonConstraintsVec &pcs,
								const mpac_generic::Pose2dContainerInterface &poses) {
  ...
  void savePositionConstraintsTextFile(const constraint_extract::PolygonConstraintsVec &pcs, const std::string &fileName) {
    ...
    for (size_t i = 0; i < pcs.size(); i++) {
      ofs << mpac_generic::getPoint2dContainerInterfaceGnuplotString(pcs[i].getInnerConstraint(), true);
...
mpac_geometry::Polygon computeSweepArea(const mpac_generic::PathInterface &path,
                                          const mpac_geometry::RobotModel2dInterface &model,
                                          const mpac_generic::RobotInternalState2d &state)
{
  mpac_geometry::Polygon sweep_area;
...
void minIncrementalDistancePathConstraints(mpac_generic::Path &path, 
...
void minIntermediateDirPathConstraintsPoints(mpac_generic::Path &path,
                                             constraint_extract::PolygonConstraintsVec &constraints)
{
...
void makeValidPathConstraintsForTrajectoryProcessing(mpac_generic::Path &path, constraint_extract::PolygonConstraintsVec &constraints) {
...
int getConstraintsIdxWithLargestDistance(const constraint_extract::PolygonConstraintsVec &constraints, const mpac_generic::Pose2d &pose, double &distance) {
...
reassignConstraints(const mpac_generic::Path &path, const constraint_extract::PolygonConstraintsVec &constraints) {
