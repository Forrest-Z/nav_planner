...
inline void metricToPixelOccupancyGrid(const nav_msgs::OccupancyGrid &msg, const Eigen::Vector2d &position, Eigen::Vector2i &pixel) {
  ...
  mpac_generic::Pose2d map_offset = mpac_conversions::createPose2dFromMsg(msg.info.origin);
...
inline bool isOccupiedCollisionInterfaceWithBoundingBox(const nav_msgs::OccupancyGrid &map, const mpac_generic::Point2dCollisionCheckInterface &collision, const Eigen::Vector2d &bottomLeft, const Eigen::Vector2d &topRight) {
...
// TODO, fix the erasing(!)
 ...
 inline mpac_generic::Point2dVec getPoint2dVecFromGridPatchIdx(const nav_msgs::OccupancyGrid &map, const GridPatchIdx &idx) {
 ...
 inline GridPatchIdx getGridPatchIdxFromPoint2dContainerInterface(const nav_msgs::OccupancyGrid &map, const mpac_generic::Point2dContainerInterface &pts) {
   GridPatchIdx ret;
 ...
 inline bool isOccupiedRobotModel2dWithState(const nav_msgs::OccupancyGrid &map, const mpac_geometry::RobotModel2dWithState &model) {
  Eigen::Vector2d bottom_left, top_right;
  mpac_geometry::getBoundingBox2d(model, bottom_left, top_right);
  
  return isOccupiedCollisionInterfaceWithBoundingBox(map, model, bottom_left, top_right);
}


 inline GridPatchIdx getCollisionInterfaceWithBoundingBoxGridPatchIdx(const nav_msgs::OccupancyGrid &map, const mpac_generic::Point2dCollisionCheckInterface &collision, const Eigen::Vector2d &bottomLeft, const Eigen::Vector2d &topRight) {
   GridPatchIdx ret;
 ...
 inline GridPatchIdx getPolygonGridPatchIdx(const nav_msgs::OccupancyGrid &map, const mpac_geometry::Polygon &poly) {
   GridPatchIdx ret;
 ...
 inline GridPatchIdx getRobotModel2dWithStateGridPatchIdx(const nav_msgs::OccupancyGrid &map, const mpac_geometry::RobotModel2dWithState &model) {
   Eigen::Vector2d bottom_left, top_right;
   mpac_geometry::getBoundingBox2d(model, bottom_left, top_right);
...
struct StateInterval {
 ...
 void computeGridThetaPatchIdx(const mpac_generic::Pose2d &origin, const nav_msgs::OccupancyGrid &map, const mpac_geometry::RobotModel2dInterface &model, const mpac_generic::RobotInternalState2d &internalState, std::vector<GridPatchIdx> &idx) {
   ...
   mpac_geometry::RobotModel2dWithState robot(model);
   
   mpac_generic::Pose2d pose = origin;
...
// Note this do not currently handle different steering angles (PHI), for waste actuated vehicles the footprint will change based on the steering angle.
 void precomputeAllowedPositions(const nav_msgs::OccupancyGrid &map,
				 ...
				 const mpac_generic::RobotInternalState2d &internalState,
   ...
   double robot_min_radius = mpac_geometry::getMinDistanceFromOrigin(model.getBoundingRegion(internalState));
   double robot_max_radius = mpac_geometry::getMaxDistanceFromOrigin(model.getBoundingRegion(internalState));
   ...
   mpac_generic::Pose2d map_offset = mpac_conversions::createPose2dFromMsg(map.info.origin);
...
inline void addPolygonToOccupancyMap(const mpac_geometry::Polygon &poly, nav_msgs::OccupancyGrid &msg, int occupancyValue) {
  // Compute the bounding box.
  Eigen::Vector2d bottom_left;
  Eigen::Vector2d top_right;
  mpac_geometry::getBoundingBox2d(poly, bottom_left, top_right);
