...
const static int AVOID_PASS = 0;
const static int AVOID_STOP = 1;
...
namespace mpac_generic
{
  ...
  {
  public:
    PointType() : direction_(0), forbid_(false), limit_(false), fixed_(false), forward_(true), max_speed_(2.0),offset_(2.0){};
    PointType(int direction, double speed, bool forbid, bool limit, bool fixed, bool forward) : direction_(direction),
                                                                                                max_speed_(speed),
                                                                                                forbid_(forbid),
                                                                                                limit_(limit),
                                                                                                fixed_(fixed),
  ...
  public:
    ...
    RobotInternalState2d()
    {
      loadType = RobotInternalState2d::NO_LOAD;
      steeringAngle = 0.;
      speed = 0.;
    }
    LoadType loadType;
  ...
  public:
    ...
    State2d(double x, double y, double th, double phi)
    {
      pose[0] = x;
      pose[1] = y;
      pose[2] = th;
    ...
    State2d(const Pose2d &p, const double &s)
    {
      pose = p;
      steeringAngle = s;
    }
    State2d(const State2dInterface &s)
    {
      pose = s.getPose2d();
      steeringAngle = s.getSteeringAngle();
    }
    State2d(const PathInterface &path, size_t idx)
    {
      pose = path.getPose2d(idx);
      steeringAngle = path.getSteeringAngle(idx);
    }
    ...
    {
      State2d ret;
      ret.pose = s * this->pose;
      ret.steeringAngle = s * this->steeringAngle;
      return ret;
    }
  ...
  public:
    ...
    void addPathPoint(const Pose2d &pose, const double &steeringAngle)
    {
      poses.push_back(pose);
      steeringAngles.push_back(steeringAngle);
      PointType pt;
      pointTypes.push_back(pt);
    }
    void addPathPointType(const Pose2d &pose, const double &steeringAngle, const PointType &pointType)
    {
      poses.push_back(pose);
      steeringAngles.push_back(steeringAngle);
    ...
    void setPathPointType(size_t idx,  PointType pointType)
    {
      pointTypes[idx] = pointType;
    }
    ...
    void setState2d(const State2dInterface &s, size_t idx)
    {
      poses[idx] = s.getPose2d();
      steeringAngles[idx] = s.getSteeringAngle();
    }
  };

  //! Path
  class PathWithType : public Path
  {
  public:
    PathWithType() {}
    PathWithType(const PathInterface &path)
    {
      for (size_t i = 0; i < path.sizePath(); i++)
      {
        PointType pt;
        this->addPathPoint(path.getPose2d(i), path.getSteeringAngle(i), pt);
      }
    }
    std::vector<PointType> pointTypes;
    void clear()
    ...
    void addPathPoint(const Pose2d &pose, const double &steeringAngle, const PointType type)
    {
      poses.push_back(pose);
      steeringAngles.push_back(steeringAngle);
      pointTypes.push_back(type);
    }
    void addPathPointInterface(const Pose2dInterface &pose, const SteeringAngleInterface &steeringAngle, const PointType type) { this->addPathPoint(pose.getPose2d(), steeringAngle.getSteeringAngle(), type); }
    void addState2dInterface(const State2dInterface &state, const PointType type) { this->addPathPointInterface(state, state, type); }
    ...
    State2d getState2d(size_t idx) const { return State2d(*this, idx); }
    PointType getPointType(size_t idx) const { return pointTypes[idx]; }
    void setState2d(const State2dInterface &s, size_t idx, PointType type)
    {
      poses[idx] = s.getPose2d();
      steeringAngles[idx] = s.getSteeringAngle();
      pointTypes[idx] = type;
  ...
  public:
    ...
    void addTrajectoryPoint(const Pose2d &pose, const double &steeringAngle, const double &fwdVel, const double &rotVel)
    {
      poses.push_back(pose);
      steeringAngles.push_back(steeringAngle);
      driveVels.push_back(fwdVel);
      steeringVels.push_back(rotVel);
    }
    ...
    State2d getState2d(size_t idx) const
    {
      State2d s(getPose2d(idx), getSteeringAngle(idx));
      return s;
    }
    Control getControl(size_t idx) const
    {
      Control c(getDriveVel(idx), getSteeringVel(idx));
      return c;
    }
  };
  ...
  {
    bool collision(const mpac_generic::Pose2d &pose) const
    {
      return false;
    }
    bool collision(const mpac_generic::Pose2d &pose, double &minDist) const
    {
      return false;
    }
  };
  ...
  public:
    ...
    void createHoles(size_t holeWidth)
    {
      ...
      while (i < this->size() - 1)
      { // Make sure that ct[size-1] is not changed.
        hole_counter++;
