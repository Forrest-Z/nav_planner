...
inline mpac_generic::CoordinatedTimes computeCoordinatedTimesFromDeltaTs(const std::vector<double> &times)
{
  mpac_generic::CoordinatedTimes ret;
  if (times.empty())
    return ret;

  ret.push_back(0.);
  for (size_t i = 0; i < times.size() - 1; i++)
  { // The last DT is not valid - just to keep the sizes equal(!).
