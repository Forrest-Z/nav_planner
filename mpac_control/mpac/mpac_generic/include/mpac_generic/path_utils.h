...
inline void removeThNormalization(PathInterface &path)
{
  ...
  for (size_t i = 1; i < path.sizePath(); i++)
  {
    ...
    mpac_generic::Pose2d pose = path.getPose2d(i);
    pose(2) += offset;
    path.setPose2d(pose, i);
  }
}

inline mpac_generic::Path getThNormalizationFreePath(const PathInterface &path)
{
  mpac_generic::Path ret(path);
...
inline mpac_generic::Path forwardSimulation(const TrajectoryInterface &traj, double len, double dt) {
  mpac_generic::Path path;
  assert(traj.sizeTrajectory() > 0);
  assert(len != 0);
  assert(dt > 0);
  mpac_generic::State2d current_state(traj, 0);
  
  path.addPathPoint(traj.getPose2d(0), traj.getSteeringAngle(0));

  for (size_t i = 0; i < traj.sizeTrajectory(); i++) {
    mpac_generic::Control ctrl(traj.getDriveVel(i), traj.getSteeringVel(i));;
...
inline mpac_generic::Path selectPathIntervall(const mpac_generic::PathInterface &path, 
                                         size_t startIdx, 
                                         size_t stopIdx) {
...
inline mpac_generic::Trajectory selectTrajectoryInterval(const TrajectoryInterface &traj,
						     size_t startIdx,
						     size_t stopIdx) {
  assert(stopIdx <= traj.sizePath());
  assert(startIdx < stopIdx);
  mpac_generic::Trajectory ret;
  for (size_t i = startIdx; i < stopIdx; i++) {
...
inline mpac_generic::Trajectory selectTrajectoryIndexes(const TrajectoryInterface &traj,
...
inline int findState(const PathInterface &states, mpac_generic::State2dInterface &query, double maxDistOffset, double maxAngularOffset, double maxSteeringAngleOffset)
{
  int ret = -1;
...
inline std::vector<std::pair<size_t, double> > getDistancesTo(const PathInterface &states, mpac_generic::State2dInterface &query) {
...
inline std::vector<std::pair<size_t, double> > findClosestStatesVec(const PathInterface &states, mpac_generic::State2dInterface &query) {
...
inline double avgSqrDistancePathError(const PathInterface &p1, const PathInterface &p2) 
{
  ...
  for (size_t i = 0; i < size; i++)
  {
    double dist = mpac_generic::getDistBetween(p1.getPose2d(i), p2.getPose2d(i));
...
inline double calcTrajectoryErrorFixedDt(const mpac_generic::TrajectoryInterface &traj, double len, double dt) {
  mpac_generic::Path p = forwardSimulation(traj, len, dt);
  //  mpac_generic::saveTrajectoryTextFile(traj, "dbg_t.txt");
  //  mpac_generic::savePathTextFile(p, "dbg_p.txt");
      
  return avgSqrDistancePathError(p, traj);
}

inline Trajectory setFixedControlValues(const mpac_generic::Path &path, double v, double w) {
  Trajectory traj;
  for (size_t i = 0; i < path.sizePath(); i++) 
  {
...
inline Trajectory setFixedControlValuesW(const mpac_generic::TrajectoryInterface &trajectory, double w) {
  Trajectory traj;
  for (size_t i = 0; i < traj.sizeTrajectory(); i++) 
  {
...
inline Path minIntermediateDirPathPointsIdx(const PathInterface &path, std::vector<size_t> &inter_idx) {
  ...
  for (size_t i = 0; i < path.sizePath(); i++) {
    if (i == inter_idx[j]) {
      ...
      State2d curr(path, i);
      State2d ds = mpac_generic::subState2d(last, curr);
...
inline bool validPathForTrajectoryProcessing(const mpac_generic::PathInterface &path) {
  mpac_generic::Path p = mpac_generic::minIntermediateDirPathPoints(path);
  return (p.sizePath() == path.sizePath());
}

inline TrajectoryChunks splitToTrajectoryChunks(const mpac_generic::TrajectoryInterface &traj, int splitStep)
{
...
inline size_t getCurrentChunkIdx(const TrajectoryChunksInterface &chunks, const mpac_generic::Pose2d &pose, size_t previousIdx) 
{
  ...
  double d1 = mpac_generic::getDistBetween(pose, chunks.getChunk(previousIdx).getPose2d(0));
  double d2 = d1+1.;

  if (previousIdx+1 < chunks.sizeChunks()) {
    d2 = mpac_generic::getDistBetween(pose, chunks.getChunk(previousIdx+1).getPose2d(0));
  }
  if (d2 < d1) {
    return previousIdx+1;
  }
  if (previousIdx+2 < chunks.sizeChunks()) {
    d2 = mpac_generic::getDistBetween(pose, chunks.getChunk(previousIdx+2).getPose2d(0));
  }
  if (d2 < d1) {
...
inline size_t getCurrentIdxSingleStep(const PathInterface &path, const mpac_generic::Pose2d &pose, size_t previousIdx) {
  if (path.sizePath() == 0)
    return 0;
  if (!(path.sizePath() > previousIdx))
    return 0;

  double d1 = mpac_generic::getDistBetween(pose, path.getPose2d(previousIdx));
  double d2 = d1+1.;
  //double d3 = d2+1.;
  // double d4 = d3+1.;
  // double d5 = d4+1.;
  ...
  if (previousIdx+1 < path.sizePath()) {
    d2 = mpac_generic::getDistBetween(pose, path.getPose2d(previousIdx+1));
  }
  if (d2 < d1) {
    return previousIdx+1;
  }
  ...
  if (previousIdx+2 < path.sizePath()) {
    d2 = mpac_generic::getDistBetween(pose, path.getPose2d(previousIdx+2));
    if (d2 < d1) {
      return previousIdx+2;
    }
  }
  ...
  //   d3 = mpac_generic::getDistBetween(pose, path.getPose2d(previousIdx+3));
  //   if (d3 < d1) {
  //     return previousIdx+3;
  //   }
  // }
  // if (previousIdx+4 < path.sizePath()) {
  //   d4 = mpac_generic::getDistBetween(pose, path.getPose2d(previousIdx+4));
  //   if (d4 < d1) {
  //     return previousIdx+4;
  //   }
  // }
  // if (previousIdx+5 < path.sizePath()) {
  //   d5 = mpac_generic::getDistBetween(pose, path.getPose2d(previousIdx+5));
  //   if (d5 < d1) {
...
inline size_t getCurrentIdx(const PathInterface &path, const mpac_generic::Pose2d &pose, size_t previousIdx) {
...
inline int getPathIdxUsingFirstPointInChunk(const PathInterface &path, const TrajectoryChunksInterface &chunks, size_t chunkIdx, double minValidDistance, size_t stepIdx) {
  ...
  for (int i = 0; i < (int)(chunkIdx-1); i++) {
    ...
    acc_chunk_dist += mpac_generic::getDistBetween(chunks.getChunk(i).getPose2d(last), chunks.getChunk(i+1).getPose2d(0));
  }
  for (int i = stepIdx; i < (int)(path.sizePath()-1); i++) {
    acc_path_dist += mpac_generic::getDistBetween(path.getPose2d(i), path.getPose2d(i+1));
...
inline mpac_generic::Trajectory trajectoryChunksInterfaceToTrajectory(const TrajectoryChunksInterface &chunks,
...
inline int getPathIdxUsingFirstPointInChunk2(const PathInterface &path, const TrajectoryChunksInterface &chunks, size_t chunkIdx, double minValidDistance, size_t stepIdx) {
  ...
  int closest_idx = mpac_generic::findState(path, chunk_state, 0.01, 0.01, 0.01);
  // return cosest_idx;

  // Additional check using the accumulated distance
  mpac_generic::Path chunk_path;
  // Push back all the states until we reach chunkIdx.
  for (size_t i = 0; i < chunkIdx; i++) {
    mpac_generic::addPathToPath(chunk_path, chunks.getChunk(i));
  }
...
inline bool checkDirection(const mpac_generic::State2d &state2d, const mpac_generic::PathInterface &path, size_t idx) {
  if (!(idx+1 < path.sizePath()))
    return false;
  bool dir1 = mpac_generic::forwardDirection(state2d.getPose2d(), 
                                               path.getPose2d(idx));
  bool dir2 = mpac_generic::forwardDirection(path.getPose2d(idx),
...
inline void addPose2dOffset(Pose2dContainerInterface &poses, mpac_generic::Pose2d &offset) {
...
inline void moveToOrigin(Pose2dContainerInterface &poses, const mpac_generic::Pose2d &origin) {
  if (poses.sizePose2d() == 0)
    return;
...
inline int getMinDistIdxToPath(const PathInterface &path, const Pose2d &pose) {
  ...
  int min_dist_idx = -1;
  for (size_t i = 0; i < path.sizePath(); i++) {
    double dist = mpac_generic::getDistBetween(path.getPose2d(i), pose);
    if (min_dist > dist) {
      min_dist = dist;
      min_dist_idx = i;
    }
...
inline double timeToReachChunkIdx(const mpac_generic::TrajectoryChunks &chunks,
                           int chunkIdx,
                           const mpac_generic::State2d &state)
{
  ...
  mpac_generic::Pose2d pose = state.getPose2d();

  while (chunk_idx > 0) {
    chunk_idx--;

    for (size_t i = 0; i < chunks.getChunk(chunk_idx).sizeTrajectory(); i++) {
      double dist = mpac_generic::getDistBetween(chunks.getChunk(chunk_idx).getPose2d(i), pose);
      if (min_dist > dist) {
        min_dist = dist;
        min_dist_idx = i;
...
inline mpac_generic::Path createStraightPath(const mpac_generic::Pose2d &start, 
                                        const mpac_generic::Pose2d &goal,
                                        double resolution)
{
...
inline mpac_generic::Path createStraightPathFromStartPose(const mpac_generic::Pose2d &start,
                                                     const mpac_generic::Pose2d &goal,
                                                     double resolution) {
  // Compute a straight line path between the points.
  mpac_generic::Path path;
  ...
  double distance = mpac_generic::getDistBetween(start, goal);
  if (!mpac_generic::forwardDirection(start, goal))
    distance *= -1;

  mpac_generic::Pose2d offset(distance, 0, 0);
  mpac_generic::Pose2d new_goal = mpac_generic::addPose2d(start, offset);
  Eigen::Vector3d diff = subPose2d(start, new_goal);
  Eigen::Vector3d inc = diff*resolution;
      
  path.addPathPoint(start, 0.);
  for (int i = 1; i <= steps; i++) {
    mpac_generic::Pose2d p = mpac_generic::addPose2d(start, i*inc);
...
inline mpac_generic::Control computeAvgSqrControlDifference(const mpac_generic::TrajectoryInterface &ref, 
                                                       const mpac_generic::TrajectoryInterface &exec) {
  double dv_sum = 0;
  double sv_sum = 0;
  ...
  mpac_generic::Control c;
  c.v = dv_sum / (ref.sizeTrajectory()*1.);
  c.w = sv_sum / (ref.sizeTrajectory()*1.);
  return c;
}

inline bool validPath(const mpac_generic::PathInterface &path, double maxSteeringAngle) {
  if (path.sizePath() == 0)
    return false;
  // Simply check for nans...
  for (size_t i = 0; i < path.sizePath(); i++) {
    const mpac_generic::Pose2d& pose = path.getPose2d(i);
...
inline bool validPathIncrSteps(const mpac_generic::PathInterface &path, double maxDistIncr, double maxThIncr, double maxSteeringAngleIncr) {
...
inline bool validSmoothedPath(const mpac_generic::PathInterface &path, 
                       const mpac_generic::State2d &start,
                       const mpac_generic::State2d &goal,
  ...
  if (!mpac_generic::validPose2dDiff(start.getPose2d(), path.getPose2d(0), maxDistOffset, maxHeadingOffset)) {
    std::cerr << "invalid smoothed path -> start pose difference to large" << std::endl;
    return false;
  }
  
  if (!mpac_generic::validPose2dDiff(goal.getPose2d(), path.getPose2d(path.sizePath()-1), maxDistOffset, maxHeadingOffset)) {
...
inline mpac_generic::Path getRepositioningPath(const mpac_generic::PathInterface &path,
  ...
  mpac_generic::Path snippet_backward = getReversePathWithoutChangingDirection(selectPathIntervall(path, idx+1, path.sizePath()));

  mpac_generic::Path snippet_forward = selectPathIntervall(path, idx, path.sizePath());
...
inline double getTotalTurning(const mpac_generic::PathInterface &path) {
  double tot = 0.;
  for (size_t i = 0; i < path.sizePath()-1; i++)
  {
...
inline int getDockingPathIdx(const mpac_generic::PathInterface &path, 
                      size_t minIdx,
...
inline void makeValidPathForTrajectoryProcessing(mpac_generic::Path &path) {
  path = mpac_generic::minIncrementalDistancePath(path, 0.00002);
  path = mpac_generic::minIntermediateDirPathPoints(path);
  //  path = mpac_generic::minIncrementalDistancePath(path, 0.00002);
}

inline void minIncrementalDistancePathCoordinatedTimes(mpac_generic::Path &path, 
                                                mpac_generic::CoordinatedTimes &cts, 
                                                double minDistance)
{
  std::vector<size_t> idx;
  path = mpac_generic::minIncrementalDistancePathIdx(path, minDistance, idx);

  if (cts.empty())
    return;

  mpac_generic::CoordinatedTimes c;
...
inline void minIntermediateDirPathCoordinatedTimesPoints(mpac_generic::Path &path,
                                                   mpac_generic::CoordinatedTimes &cts)
{
  std::vector<size_t> inter_idx;
  path = mpac_generic::minIntermediateDirPathPointsIdx(path, inter_idx);
  if (inter_idx.empty())
    return;
...
inline void makeValidPathCoordinatedTimesForTrajectoryProcessing(mpac_generic::Path &path, mpac_generic::CoordinatedTimes &cts) {
...
inline double coordinationPairTimeStep(const mpac_generic::CoordinatedTimes &cts, size_t idx) {
  if (cts.size() > idx) {
