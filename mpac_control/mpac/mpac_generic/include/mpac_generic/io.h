...
namespace mpac_generic
{
  ...
  inline std::vector<mpac_generic::Path> loadPathDirTextFile(const std::string &dirName) {
	...
	mpac_generic::Path p = mpac_generic::loadPathTextFile(file_names[i]);
	if (p.sizePath() == 0)
	  {
  ...
  inline void saveQuaternionPathTextFile(const PathInterface &path, const std::string &fileName) { 
    ...
    for (unsigned int i = 0; i < path.sizePath(); i++)
      {
	Eigen::Quaterniond q = mpac_generic::getQuaterion(path.getPose2d(i));
	ofs << path.getPose2d(i)(0) << " "
	    << path.getPose2d(i)(1) << " "
	    << q.z() << " "
	    << q.w() << " "
  ...
  inline void saveTraversedDistanceTrajectoryTextFile(const TrajectoryInterface &traj, const std::string &fileName) {
	...
	dist_sum += mpac_generic::getDistBetween(traj.getPose2d(i), traj.getPose2d(i+1));
      }
    }	  
  ...
  inline CoordinatedTimes loadCoordinatedTimesTextFile(const std::string &fileName) {
    std::vector<double> tmp = mpac_generic::loadDoubleVecTextFile(fileName);
