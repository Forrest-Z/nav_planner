...
void drawPose2d(const mpac_generic::Pose2d &pose, int id, ros::Publisher &pub)
{
  std::string name = "pose2d";
  drawPose2d(pose(0), pose(1), pose(2), id, id, 0.5, name, pub);
}

void drawPose2d(const mpac_generic::Pose2d &pose, int id, const std::string &name, ros::Publisher &pub)
{
  drawPose2d(pose(0), pose(1), pose(2), id, id, 0.5, name, pub);
}

void drawPose2d(const mpac_generic::Pose2d &pose, int id, int color, const std::string &name, ros::Publisher &pub)
{
  drawPose2d(pose(0), pose(1), pose(2), id, color, 0.5, name, pub);
}

 void drawPose2d(const mpac_generic::Pose2d &pose, int id, int color, double scale, const std::string &name, ros::Publisher &pub)
{
...
void drawCovariance(const mpac_generic::Pose2dCov &poseCov, visualization_msgs::Marker &marker)
{
     drawCovariance(poseCov.mean, poseCov.cov, marker);
}

void drawPose2dCov(const mpac_generic::Pose2dCov &pose, int id, int color, double sigma, const std::string &name, ros::Publisher &pub) 
{
...
void drawPose2d(const mpac_generic::Pose2dContainerInterface &poses, int idx, int id, int color, double scale, const std::string &name, ros::Publisher &pub) {
  if (idx < 0)
    return;
...
void drawPose2dContainer(const mpac_generic::Pose2dContainerInterface &poses, const std::string &name, int id, ros::Publisher &pub)
{
   visualization_msgs::Marker m;
   assignDefault(m);
   assignColor(m, id);
  ...
  for (unsigned int i = 0; i < poses.sizePose2d(); i++)
    {
      m.id = i;
      const mpac_generic::Pose2d &p = poses.getPose2d(i);
...
void drawPoint2dContainerAsConnectedLineIncZ_(const mpac_generic::Point2dContainerInterface &points, const std::string &name, int id, int color, double startZ, double incZ, bool addLastPointFirst, ros::Publisher &pub) {
  ...
  m.action = visualization_msgs::Marker::ADD;
  m.ns = name + mpac_generic::toString(id);
  m.id = id;
 ...
 void drawPoint2dContainerAsConnectedLineIncZ(const mpac_generic::Point2dContainerInterface &points, const std::string &name, int id, int color, double startZ, double incZ, ros::Publisher &pub) {
   drawPoint2dContainerAsConnectedLineIncZ_(points, name, id, color, startZ, incZ, true, pub);
 }

 void drawPoint2dContainerAsConnectedLine(const mpac_generic::Point2dContainerInterface &points, const std::string &name, int id, int color, ros::Publisher &pub)
{
  drawPoint2dContainerAsConnectedLineIncZ(points, name, id, color, 0., 0., pub);
}

 void drawPathInterfaceIncZ(const mpac_generic::PathInterface &path, const std::string &name, int id, int color, double incZ, ros::Publisher &pub)
 {
   mpac_generic::Point2dVec pvec = mpac_generic::createPoint2dVecFromPose2dContainerInterface(path);
...
std::vector<Eigen::Vector2d> getIntersectionsPose2d(const std::vector<double> &A0, const std::vector<double> &A1, const std::vector<double> &b, const mpac_generic::Pose2d& start, const mpac_generic::Pose2d& end, double bOffset)
{
  std::vector<Eigen::Vector2d> intersections;
  double A0_, A1_, b_;
  for (unsigned int i = 0; i < A0.size(); i++) {
    
    calcMatrixFormForLine(mpac_generic::getPosition(start), mpac_generic::getBaseOffset(start, 1.), A0_, A1_, b_);
    intersections.push_back(getIntersection(A0[i], A1[i], A0_, A1_, b[i]+bOffset, b_));
    
    calcMatrixFormForLine(mpac_generic::getPosition(end), mpac_generic::getBaseOffset(end, 1.), A0_, A1_, b_);
    intersections.push_back(getIntersection(A0[i], A1[i], A0_, A1_, b[i]+bOffset, b_));
  }
 ...
 void drawPolygonConstraint(const mpac_msgs::PolygonConstraint &msg, visualization_msgs::Marker m, ros::Publisher &pub, double bOffset)
{
 ...
 void drawConstraintsMarker(const mpac_msgs::RobotConstraints &msg, visualization_msgs::Marker m, visualization_msgs::Marker m2, ros::Publisher &pub, int stepSize)
{
  m.type = visualization_msgs::Marker::LINE_STRIP;
  m.action = visualization_msgs::Marker::ADD;
  m.ns = "constraints_" + mpac_generic::toString(msg.robot_id) + mpac_generic::toString(msg.goal_id);
  m2.type = visualization_msgs::Marker::LINE_STRIP;
  m2.action = visualization_msgs::Marker::ADD;
  m2.ns = "constraints2_" + mpac_generic::toString(msg.robot_id) + mpac_generic::toString(msg.goal_id);
...
void drawConstraints(const mpac_msgs::RobotConstraints &msg, ros::Publisher &pub)
{
...
void drawSteeringWheel(const mpac_generic::Pose2d &pose, const std::string &name, int id, ros::Publisher &pub)
 {
   visualization_msgs::Marker m;
   assignDefault(m);
   assignColor(m, 2);
 ...
 void drawPathInterface(const mpac_generic::PathInterface &path, const std::string &name, int id, double L, ros::Publisher &pub)
 {
   // Draw the pose
   ...
   for (unsigned int i = 0; i < path.sizePath(); i++)
     {
       mpac_generic::Pose2d origin = path.getPose2d(i);
       mpac_generic::Pose2d offset;
       offset << L, 0, path.getSteeringAngle(i);
       mpac_generic::Pose2d sw = mpac_generic::addPose2d(origin, offset);
       drawSteeringWheel(sw, name, i, pub);
     }
}

 void drawPositionContainer(const mpac_generic::PositionContainerInterface &points, double scale, const std::string &name, int id, ros::Publisher &pub)
 {
    visualization_msgs::Marker m;
     assignDefault(m);
     assignColor(m, id);
 ...
 void drawPositionContainer(const mpac_generic::PositionContainerInterface &points, const std::string &name, int id, ros::Publisher &pub)
 {
   drawPositionContainer(points, 0.2, name, id, pub);
 }

 void drawPositionContainerAsLineList(const mpac_generic::PositionContainerInterface &points, double scale, const std::string &name, int id, ros::Publisher &pub)
 {
    visualization_msgs::Marker m;
     assignDefault(m);
     assignColor(m, id);
 ...
 void drawTrajectoryWithControlTypeSpeed(const mpac_generic::TrajectoryInterface &traj, int color, int id, int type, double scale, const std::string &name, bool useSteeringVel, ros::Publisher &pub) {
  visualization_msgs::Marker m;
  assignDefault(m);
  assignColor(m, color);
 ...
 void drawSteeringAngleVel(const mpac_generic::TrajectoryInterface &traj, int color, const std::string &name, ros::Publisher &pub) {
   drawTrajectoryWithControlTypeSpeed(traj, color, 1, 4, 0.02, name, true, pub);
 }

 void drawTrajectoryWithControlType(const mpac_generic::TrajectoryInterface &traj, int color, int id, int type, double scale, const std::string &name, ros::Publisher &pub) {
 ...
 void drawTrajectoryWithControl(const mpac_generic::TrajectoryInterface &traj, int color, int id, const std::string &name, ros::Publisher &pub) {
   // Draw the trajectory as a line which follows the path using [x,y] and the speed is the z value.
   drawTrajectoryWithControlType(traj, color, id, 4, 0.03, name, pub);
 }

 void drawTrajectoryChunksWithControl(const mpac_generic::TrajectoryChunksInterface &chunks, int color, const std::string &name, ros::Publisher &pub) {
...
void drawTrajectoryChunksUsingTime(const mpac_generic::TrajectoryChunksInterface &chunks, int color, const std::string &name, double timeOffset, double timeToMeterFactor, double dt, ros::Publisher &pub) {
 ...
 void drawPalletWithTime(const mpac_generic::Pose2d &pose, const std::string &frame, int id, const ros::Time &time, ros::Publisher &pub) {
...
void drawPallet(const mpac_generic::Pose2d &pose, const std::string &frame, int id, ros::Publisher &pub) {
  drawPalletWithTime(pose, frame, id, ros::Time::now(), pub);
}

void drawText(const mpac_generic::Pose2d &pose, const std::string &text, const std::string &name, int id, int color, double height, double size, ros::Publisher &pub) {
   visualization_msgs::Marker m;
   assignDefault(m);
   assignColor(m, color);
   m.pose.position.x = pose(0);
...
void drawPose2dTimes(const mpac_generic::Pose2dContainerInterface &poses, const std::vector<double> &times, double startTime, const std::string &name, int color, double timeScale, double sizeScale, ros::Publisher &pub) {
  mpac_generic::CoordinatedTimes ct(times);
...
void drawPose2dTimesAsLines(const mpac_generic::Pose2dContainerInterface &poses, const std::vector<double> &times, double startTime, const std::string &name, int color, double timeScale, double sizeScale, ros::Publisher &pub) {
mpac_generic::CoordinatedTimes ct(times);
  ct.addOffset(-startTime);
  assert(poses.sizePose2d() == times.size());
  mpac_generic::PositionVec pts;
...
void drawPose2dTimes(const mpac_generic::Pose2dContainerInterface &poses, const std::vector<double> &times, double startTime, const std::string &name, int color, double timeScale, ros::Publisher &pub) {
  drawPose2dTimes(poses, times, startTime, name, color, timeScale, 0.05, pub);
}


void drawCoordinatedTimes(const mpac_generic::Pose2dContainerInterface &poses, const mpac_generic::CoordinatedTimes &coordTimes, double startTime, const std::string &name, int color, double timeScale, double sizeScale, ros::Publisher &pub) {
  drawPose2dTimes(poses, coordTimes, startTime, name, color, timeScale, sizeScale, pub);
}

void drawCoordinatedTimes(const mpac_generic::Pose2dContainerInterface &poses, const mpac_generic::CoordinatedTimes &coordTimes, double startTime, const std::string &name, int color, double timeScale, ros::Publisher &pub) {
  drawPose2dTimes(poses, coordTimes, startTime, name, color, timeScale, pub);
}

void drawDeltaTVec(const mpac_generic::Pose2dContainerInterface &poses, const mpac_msgs::DeltaTVec &msg, ros::Publisher &pub) {
  for (size_t i = 0; i < msg.dts.size(); i++) {
    std::vector<double> dts = mpac_conversions::getDoubleVecFromDeltaTMsg(msg.dts[i]);
    mpac_generic::CoordinatedTimes ct = mpac_generic::computeCoordinatedTimesFromDeltaTs(dts);
 ...
 void drawTrajectoryLinePairs(const mpac_generic::TrajectoryInterface &traj1, const mpac_generic::TrajectoryInterface &traj2, int color, int id, const std::string &name, ros::Publisher &pub) {
 ...
 void drawTrajectoryRefAndExecPair(const mpac_generic::TrajectoryInterface &ref, const mpac_generic::Trajectory &exec, const std::string &name, ros::Publisher &pub) {
...
#if 0
void drawTrajectoryMarker(const mpac_msgs::Trajectory &msg, visualization_msgs::Marker m, ros::Publisher &pub, int stepSize)
{
  m.type = visualization_msgs::Marker::ARROW;
  m.action = visualization_msgs::Marker::ADD;
