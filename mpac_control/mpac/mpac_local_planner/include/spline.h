...
public:
    ...
    double calcd(double x) const;
    double calcdd(double x) const;
...
double spline::calcd(double x) const
{
    size_t n=m_x.size();
    // find the closest point m_x[idx] < x, idx=0 even if x<m_x[0]
    std::vector<double>::const_iterator it;
    ...
    double result;
    // interpolation
    result= 3*m_a[idx] * pow(h,2) + 2*m_b[idx] * h + m_c[idx];
    return result;
}

double spline::calcdd(double x) const
{
    size_t n=m_x.size();
    // find the closest point m_x[idx] < x, idx=0 even if x<m_x[0]
    std::vector<double>::const_iterator it;
