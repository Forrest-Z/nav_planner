...
public:
	...
	/** 未知区域是否可通行 */
	static bool PASSABLE_UNKNOWN_SPACE;

	/** 机器人内切半径 */
	static float RADIUS_CAR_INSCRIBED;
	/** 机器人外切半径 */
	static float RADIUS_CAR_EXTERNAL;


	/** Define how many decimal figures should be used */
	static int DECIMAL_APPROXIMATION;
	
	/** Flag to enable/disable the use of grid distance heuristic estimation in the planners:
	 * At least one is true between USE_HEURISTIC_ESTIMATION and USE_GRID_DISTANCE 
	 **/
    static bool USE_GRID_DISTANCE;

	static bool USE_FAST_PLAN_MODE;
