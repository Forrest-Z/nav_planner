...
#include "mpac_global_planner/AStarPathPlanner.h"
...
AStarPathPlanner::AStarPathPlanner(PathNode *startNode, World *world) : AStar(startNode)
{
	...
	node_pub_ = nh.advertise<visualization_msgs::Marker>("visualization_marker_plan", 100000);
}
...
std::vector<Node *> AStarPathPlanner::solve()
{
	...
	while (solutionFound == false)
	{
		...
		if (WP::LOG_LEVEL >= 1 && progress%1== 0)
		{
			double x = candidate->getConfigurations()[0]->getXCoordinate();
			double y = candidate->getConfigurations()[0]->getYCoordinate();
			double yaw = candidate->getConfigurations()[0]->getOrientation();
			...
			// node_pub_.publish(pose);
			// ROS_INFO("candia:%f  %f  %f", x, y, yaw);
			progress = 0;
			...
			m.pose.position.y = y;
			m.pose.position.z = 0;
			m.pose.orientation = tf::createQuaternionMsgFromYaw(yaw);
			m.scale.x = 0.025;
			m.scale.y = 0.01;
			m.scale.z = 0.01;
			node_pub_.publish(m);
		...
		{
			std::cout<<"has candidate "<<id<<" time"<<std::endl;
		...
		if (WP::LOG_LEVEL >= 1 && progress == 10000)
		{
			char line[150];
			writeLogLine(line, "AStarPathPlanner", WP::LOG_FILE);
			progress = 0;
