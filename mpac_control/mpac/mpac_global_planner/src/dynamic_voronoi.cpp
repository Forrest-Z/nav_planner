...
void DynamicVoronoi::buildVoronoiFromMap(const std::vector<std::vector<int>> map_data)
{

  sizeY = map_data.size();
  sizeX = map_data[0].size();
  std::vector<std::vector<bool>> binMap;
  binMap.resize(sizeX);
  for (int x = 0; x < sizeX; x++)
  {
    binMap[x].resize(sizeY);
  }
  for (int x = 0; x < sizeX; ++x)
  {
    for (int y = 0; y < sizeY; ++y)
    {
      binMap[x][y] = (map_data[y][x]) == 0 ? false : true;
    }
  }
...
void DynamicVoronoi::buildVoronoiFromMap(const std::vector<std::vector<double>> map_data)
{
...
void DynamicVoronoi::initializeEmpty(int _sizeX, int _sizeY, bool initGridMap)
{
  ...
  std::queue<Vec2i> empty;
  swap(empty,pruneQueue);  
...
void DynamicVoronoi::visualize(const char *filename)
{
  ...
  if (!F)
  {
    std::cout << "could not open 'result.ppm' for writing!\n";
    return;
  }
  fprintf(F, "P6\n");
  fprintf(F, "%d %d 255\n", sizeX, sizeY);
