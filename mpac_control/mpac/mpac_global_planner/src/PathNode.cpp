...
#include "mpac_global_planner/PathNode.h"
...
PathNode::PathNode(std::vector<Configuration*> newConfs, std::vector<Configuration*> unchangedConfs, World* w, PathNode* parent){
	...
	H_ = h1 > h2 ? h1 : h2;
	// for anytime algorithms, the heuristic weight can be changed
	H_ = H_ * WP::HEURISTIC_VALUE_MULTIPLIER*1.02;
	F_ = G_ + H_;
}
...
bool PathNode::equalContent(Node* pn) {
	...
	// bool result = true;
	// for (size_t i = 0; i < currentConfigurations_.size(); i++)
	// {
	// 		if ((currentConfigurations_[i]->getXCell() != (dynamic_cast<PathNode*> (pn))->getConfigurations()[i]->getXCell()) || (currentConfigurations_[i]->getXCell() != (dynamic_cast<PathNode*> (pn))->getConfigurations()[i]->getXCell()) ) {
	// 			result = false;
	// 			break;
	// 		} 
