...
CarModel::CarModel(std::string modelPrimitivesFilename,const double& car_left_width,const double& car_right_width,const double& car_front_length,const double& car_back_length):VehicleModel(modelPrimitivesFilename){
		...
		while (f.good()) {
			...
			if (what[0].matched) {
				...
				for (int i = 0; i < intermediatePoints; i++) {
					...
					//-pi ~ +pi
					sp->orient = atan2(sin(sp->orient),cos(sp->orient));
	...
	this->carBackLength_ =car_back_length;
	this->carFrontLength_ = car_front_length;
	this->carLeftWidth_ = car_left_width;
	this->carRightWidth_ = car_right_width;
	this->width_ = car_left_width + car_right_width;

	motionPrimitiveAdditionalDataFilename_.append("_").append(std::to_string(this->carBackLength_))
										  .append("_").append(std::to_string(this->carFrontLength_))
										  .append("_").append(std::to_string(this->width_))
										  .append(".adat");
...
void CarModel::generatePrimitiveAdditionalData() {
	...
	printf("generatePrimitiveAdditionalData!\n");
	printf("modelMotionPrimitivesLT_:%d!\n",modelMotionPrimitivesLT_.size());
...
double  CarModel::getCarLeftWidth(){
	return carLeftWidth_;
}

double  CarModel::getCarRightWidth(){
	return carRightWidth_;
}