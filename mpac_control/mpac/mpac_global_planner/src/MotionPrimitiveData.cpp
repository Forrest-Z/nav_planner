...
#include "mpac_global_planner/MotionPrimitiveData.h"
#include <math.h>
...
void MotionPrimitiveData::setTrajectory(std::vector<vehicleSimplePoint*> trajectory,
	...
	double angle = traj_.back()->orient - traj_.front()->orient;

	angle_ = fabs(atan2(sin(angle),cos(angle)));
...
double MotionPrimitiveData::getAngleRotation() {
	return angle_;
}
