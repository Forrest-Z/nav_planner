...
#include "mpac_global_planner/World.h"
#include "ros/ros.h"
...
World::World(WorldOccupancyMap* m, std::vector<VehicleMission*> missions) {
	...
	// if (WP::USE_GRID_DISTANCE) {
	// 	ros::Time start = ros::Time::now();
	// 	calculateGridDistancesFromGoals();
	// 	ros::Time end = ros::Time::now();
	// 	SPDLOG_INFO("Calculate grid distances from goals using time:{}s",(end-start).toSec());
	// }
}

World::World(WorldOccupancyMap* m) {
...
void World::UpdateMissions(std::vector<VehicleMission*> missions){
...
void World::calculateGridDistancesFromGoals() {
	...
	for (std::vector<VehicleMission*>::iterator mit = missions_.begin(); mit != missions_.end(); mit++ ) {
		// if (!(*mit)->isNeedUpdateGridDistanceFromGoal)
		// {
		// 	printf("same goal!!!!!!!!!!!!!!!!!!!!!!!!!;\n");
		// 	continue;
		// }
				...
				for (int dy = -1; dy <= 1; dy++) {
					for (int dx = -1; dx <= 1; dx++) {
						...
						if (new_x >= 0 && new_y >= 0 && new_x < (int) gridDistances[0].size() && new_y < (int) gridDistances.size()) {
							...
							if (!this->containsObstacles() || (this->getCollisionDetector()->isPassable(new_x,new_y,parent->x,parent->y))) {
...
CollisionDetector* World::getCollisionDetector() {
	return cd_;
}
