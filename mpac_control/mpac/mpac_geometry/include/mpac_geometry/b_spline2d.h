...
#include <mpac_geometry/b_spline.h>
...
public:
  ...
  unsigned int getNbKnots() const;
    ...
    //! Return the number of knots.
  unsigned int getNbKnotsS() const;
  ...
  void setPoses(const mpac_generic::Pose2dContainerInterface &poses, double speed, double knotOffset = 1.);
  void setPosesAsKnots(const mpac_generic::Pose2dContainerInterface &poses, double speed, double knotOffset = 1.);

  // Add a single pose creating 3 knots.
  void addPose(const mpac_generic::Pose2d &pose, double speed, double knotOffset);
  // Add a single pose extracting only the position (x,y) to the knot.
  void addKnot(const mpac_generic::Pose2d &pose, double speed);
  ...
  double getS(unsigned int idx) const;
  void getKnots(mpac_generic::PositionVec &pos) const;

  //! Return a pose at the value \param t.
  mpac_generic::Pose2d getPose(double t) const;
  ...
  void calcWaypoints(mpac_generic::Pose2dVec &points, double resolution) const;

  void calcPath(mpac_generic::Path &path, double L, double resolution) const;

  mpac_generic::Pose2d getGoal() const;
  ...
  bool collision(const mpac_generic::CollisionCheckInterface &collisionCheck, double resolution) const;
  bool evaluate(BSpline2d::Evaluation &eval, double L, double minSteeringAngle, double maxSteeringAngle, double resolution) const;

private:
  void calcKnots(const mpac_generic::Pose2d &pose, std::vector<double> &x, std::vector<double> &y, double offset);
  BSpline _x;
  BSpline _y;
  BSpline _speed;
