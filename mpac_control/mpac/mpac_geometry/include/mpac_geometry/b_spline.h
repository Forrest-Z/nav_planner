...
inline bool eval_spline(double &x_val, const double &t_val, const std::vector<double> &x, const std::vector<double> &t)
{
     ...
     double t_start = ((t[0]+t[2])/2+2*t[1])/3;
     double t_end =  ((t[1]+t[3])/2+2*t[2])/3;
    double u = (t_val - t_start) / (t_end -t_start);
...
public:
     ...
     void addPointS(double x, double s);
     ...
     //! Return the value of the spline in val, returns false if the t value was out of bounds.
     bool GetValue(double &val, double s) const;
     ...
     //! Return the number of points used.
     unsigned int Size() const { return _xs.size(); }
     ...
     //! Return s value at idx.
     double getValueS(unsigned int idx) const;

private:
     bool selectKnots(std::vector<double> &x_vec, std::vector<double> &t_vec, double t) const;
     bool SelectKnots(std::vector<double> &x_vec, std::vector<double> &s_vec, double s) const;
     std::map<double, double> _xt;
     std::map<double, double> _xs;
     bool _angle;
};
