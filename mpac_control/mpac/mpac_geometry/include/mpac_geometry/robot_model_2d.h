...
namespace mpac_geometry {
  ...
  public:
     ...
     */
    virtual const Polygon& getBoundingRegion(const mpac_generic::RobotInternalState2d &s) const = 0;
  };
  ...
  public:
    ...
    virtual const Polygon& getBoundingRegion(const mpac_generic::RobotInternalState2d &s) const {
      switch (s.loadType) {
      case mpac_generic::RobotInternalState2d::NO_LOAD: 
	return wo_load;
	break;
      case mpac_generic::RobotInternalState2d::EUR_PALLET:
	// TODO!!!!
	return w_load;
	break;
      case mpac_generic::RobotInternalState2d::HALF_PALLET:
	// TODO!!!!
	return w_load2;
	break;
      default:
  ...
  public:
    RobotModel2dSnowWhite() { 
      bound.points.push_back(Eigen::Vector2d(0.48, 0.33));
      bound.points.push_back(Eigen::Vector2d(0.48, -0.33));
      bound.points.push_back(Eigen::Vector2d(-0.48, -0.33));
      bound.points.push_back(Eigen::Vector2d(-0.48, 0.33));
...
//      bound.points.push_back(Eigen::Vector2d(-0.165, 0.25));
  ...
  private:
    mpac_geometry::RobotModel2dSnowWhite model1;
    mpac_geometry::RobotModel2dCiTiTruck model2;
    mpac_geometry::RobotModel2dOneSquareMeter model3;
    mpac_geometry::RobotModel2dPitViper model4;
    mpac_geometry::RobotModel2dXa15 model5;
    mpac_geometry::RobotModel2dHRP model6;
  };

  class RobotModel2dWithState : public mpac_generic::Point2dContainerInterface, public mpac_generic::Point2dCollisionCheckInterface {
  public:
    ...
    void update(const mpac_generic::Pose2d &p, const mpac_generic::RobotInternalState2d &is);
    const Polygon& getPosePolygon() const { return posePolygon; }
    const mpac_generic::Pose2d& getRobotPose() const { return robotPose; }
    void getBoundingBox(Eigen::Vector2d &bottomLeft, Eigen::Vector2d &topRight) const {
      mpac_geometry::getBoundingBox2d(this->posePolygon, bottomLeft, topRight);
    }

  private:
    const RobotModel2dInterface& model;
    Polygon posePolygon;
    mpac_generic::Pose2d robotPose;
  };
