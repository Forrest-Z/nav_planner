#include <mpac_geometry/b_spline2d.h>
...
unsigned int BSpline2d::getNbKnots() const
{
...
unsigned int BSpline2d::getNbKnotsS() const
{
  assert(_x.Size() == _y.Size());
  assert(_x.Size() == _speed.Size());
...
void BSpline2d::setPoses(const mpac_generic::Pose2dContainerInterface &poses, double speed, double knotOffset)
{
  clear();
...
void BSpline2d::setPosesAsKnots(const mpac_generic::Pose2dContainerInterface &poses, double speed, double knotOffset)
{
  clear();
...
void BSpline2d::addPose(const mpac_generic::Pose2d &pose, double speed, double knotOffset)
{
  std::vector<double> x;
  std::vector<double> y;
  ...
  if (this->getNbKnotsS() > 0)
  {
    s = getS(getNbKnotsS() - 1) + sqrt(pow((pose.x()-_x.getValue(getNbKnots() - 1)),2)+pow((pose.y()-_y.getValue(getNbKnots() - 1)),2));
...
void BSpline2d::addKnot(const mpac_generic::Pose2d &pose, double speed)
{
  assert(_x.size() == _y.size());
  assert(_x.size() == _speed.size());
  ...
  if (this->getNbKnotsS()>0)
  {
    s = getS(getNbKnotsS() - 1);
    ds = sqrt(pow((pose.x()-_x.getValue(getNbKnots() - 1)),2)+pow((pose.y()-_y.getValue(getNbKnots() - 1)),2));
    s = s+ds;  
  }
  _x.addPointS(pose.x(),s);
  _y.addPointS(pose.y(),s);
  _speed.addPointS(speed,s);
...
void BSpline2d::setKnots(const std::vector<double> &x, const std::vector<double> &y, std::vector<double> &speed)
{
  clear();
  assert(x.size() == y.size());
...
void BSpline2d::calcKnots(const mpac_generic::Pose2d &pose, std::vector<double> &x, std::vector<double> &y, double offset)
{
  double mx = pose(0);
  double my = pose(1);
  double mth = pose(2);
...
double
BSpline2d::getS(unsigned int idx) const
{
  double s = _x.getValueS(idx);
  assert(s == _y.getValueS(idx));
  assert(s == _speed.getValueS(idx));

  return s;
}

void BSpline2d::getKnots(mpac_generic::PositionVec &pos) const
{
...
BSpline2d::getPose(double t) const
{
  ...
  if (getSpeed(t) >= 0)
    return mpac_generic::Pose2d(x, y, atan2(dy, dx));
  else
    return mpac_generic::Pose2d(x, y, angles::normalize_angle(atan2(dy, dx) + M_PI));
}

mpac_generic::Pose2d
BSpline2d::GetPose(double s) const
{
  double x, y, dx, dy;
  if (_x.GetValue(x, s) && _y.GetValue(y, s) && _x.GetValueFirstDer(dx, s) && _y.GetValueFirstDer(dy, s))
  //     if (_x.getValue(x, t) && _y.getValue(y, t) && _x.getValueFirstDerApprox(dx, t) && _y.getValueFirstDerApprox(dy, t))
  {
    // Ok.
  }
...
double BSpline2d::getSteeringAngle(double t, double L) const
{
...
double BSpline2d::getSpeed(double t) const
{
  return _speed.getValue(t);
}

bool BSpline2d::getCurvature(double &curvature, double t) const
{
  double x, xdot, xdotdot;
  double y, ydot, ydotdot;
  if (!_x.getAllValues(x, xdot, xdotdot, t))
...
bool BSpline2d::collision(const mpac_generic::CollisionCheckInterface &collisionCheck, double resolution) const
{
  double min_T = getT(1);
...
bool BSpline2d::evaluate(BSpline2d::Evaluation &eval, double L, double minSteeringAngle, double maxSteeringAngle, double resolution) const
{
...
void BSpline2d::calcWaypoints(mpac_generic::Pose2dVec &points, double resolution) const
{
  points.clear();
...
void BSpline2d::calcPath(mpac_generic::Path &path, double L, double resolution) const
{
  path.clear();
...
bool BSpline2d::getForwardHeading() const
{
  if (getSpeed(0.) >= 0)
    return true;
  return false;
