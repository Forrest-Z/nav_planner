...
void 
BSpline::addPointS(double x, double s)
{
     _xs[s] = x;
}
...
bool
BSpline::SelectKnots(std::vector<double> &x_vec, std::vector<double> &s_vec, double s) const
{
  if (_xs.empty())
    return false;
  ...
  std::map<double,double>::const_iterator it = _xs.upper_bound(s); // Should be [2].
  
  double max_S = getValueS(this->size() - 1);

  if (s == getValueS(this->size() - 1))
    {
      // The it is [3] in this case (need to simply move the iter to [2].
      it--;
    }
...
BSpline::GetValue(unsigned int idx) const
{
     ...
     std::map<double, double>::const_iterator it = _xs.begin();
...
double 
BSpline::getValueS(unsigned int idx) const
{
     assert(idx < _xt.size());
