...
bool getDeltaTVecCallback(mpac_msgs::GetDeltaTVec::Request  &req,
                          mpac_msgs::GetDeltaTVec::Response &res )
{
    ...
    for (unsigned int i = 0; i < params.size(); i++)
    {
        ...
        res.dts.dts.push_back(mpac_conversions::createDeltaTMsgFromDeltaTInterface(gen));
        std::string file_name("deltatvec" + mpac_generic::toString(i) + ".txt");
        mpac_generic::saveDeltaTInterface(gen, file_name);
    }

    if (visualize) {
      mpac_rviz::drawDeltaTVec(path, res.dts, marker_pub);
    }
