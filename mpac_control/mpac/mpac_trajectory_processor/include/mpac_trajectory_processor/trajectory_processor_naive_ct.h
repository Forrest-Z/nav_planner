...
class TrajectoryProcessorNaiveCT : public TrajectoryProcessor //, public mpac_generic::DeltaTInterface
{
 public:
 TrajectoryProcessorNaiveCT() : _startIdx(0), _startIdxControl(0.,0.), _startIdxTime(-1.) { }
  ...
  mpac_generic::Trajectory getTrajectory() {
    ...
    if (_params.debug) {
      mpac_generic::saveDoubleVecTextFile(this->_coordinatedTimes, _params.debugPrefix + std::string("tpnct_input.ct"));
      mpac_generic::savePathTextFile(this->_path, _params.debugPrefix + std::string("tpnct_input.path"));
    }
    ...
    if (_startIdx == 0) { // Starting from beginning? no special handling. - this is where CASE 2 will end up...
      ...
      // if (mpac_generic::coordinationPairTimeStep(_coordinatedTimes, _startIdx) > 3.) {
      //   ROS_ERROR_STREAM("---------------------=================== : " << mpac_generic::coordinationPairTimeStep(_coordinatedTimes, _startIdx));
  ...
  const mpac_generic::CoordinatedTimes& getCoordinatedTimes() const { return _coordinatedTimes; }
  const mpac_generic::Path& getPath() const { return _path; }
 ...
 protected:
  ...
  mpac_generic::Trajectory getTrajectoryNoCoordination() {
  ...
  mpac_generic::Trajectory getTrajectoryBeginning() {
  ...
  mpac_generic::Trajectory getTrajectoryStandStill() {
    std::cerr << "getTrajectoryStandStill()" << std::endl;
    mpac_generic::Trajectory traj;
    mpac_generic::State2d s(_path, _startIdx);
    mpac_generic::Control c(0.,0.);
  ...
  mpac_generic::Trajectory getTrajectoryNonReversing() {
	...
	_coordinatedTimes[_startIdx] = _startIdxTime;
      ...
      mpac_generic::saveDoubleVecTextFile(this->_coordinatedTimes, "tpnct_non_reversing_input.ct");
      mpac_generic::savePathTextFile(this->_path, "tpnct_non_reversing_input.path");
  ...
  mpac_generic::Trajectory getTrajectoryReversing() {
    ...
    mpac_generic::Path rev_full_path = mpac_generic::getReversePathWithoutChangingDirection(_path);
    ...
    mpac_generic::Path rev_path = mpac_generic::selectPathIntervall(rev_full_path, 0, rev_ct+1);
  ...
  size_t _startIdx;
  mpac_generic::Control _startIdxControl;

  std::vector<std::pair<size_t, mpac_generic::Control> > _controlConstraintPoints;
