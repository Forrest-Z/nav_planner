...
class ControlConstraint
{
 ...
 ControlConstraint() : valid(false) { }
  void assignControl(const mpac_generic::Control &control) { c = control; valid = true; }
...
class TrajectoryStepNaive
{
 public:
  ...
  mpac_generic::Control getControlStep() { return ds.getStateIncrControlStep(dt); }
  
  //! Return driving direction, 1 = forward, -1 = backward
  double getDir() const { 
    return dir;
    //    assert(fabs(s_point.getSteeringAngle()) < M_PI/2.);
    //    return mpac_generic::getDirectionIncr(this->ds.getPose2d());
  }

  //! Return the average fwd speed.
  //  double getVStep() const { return this->getDir()*mpac_generic::getDist(ds.getPose2d()) / dt; }
  ...
  double getPositionDiff() const { return mpac_generic::getDist(ds.getPose2d()); }
  double getHeadingDiff() const { return mpac_generic::getHeading(ds.getPose2d()); }
  double getSteeringDiff() const { return ds.getSteeringAngle(); }
  
  //! State value - never changes
  mpac_generic::State2d s_point;
  //! State value - never changes
  mpac_generic::State2d ds; // -"-
  ...
  mpac_generic::Control c_point; 
  mpac_generic::Control c_step;
...
class TrajectoryStepNaiveVec : public std::vector<TrajectoryStepNaive>, public mpac_generic::DeltaTInterface
{
 public:
  ...
  mpac_generic::Trajectory convertNaiveStepsToTrajectory() const
    {
  ...
  void saveGnuplotFile(const std::string &fileName) const
  {
	...
	const TrajectoryStepNaive &s = (*this)[i];
	const mpac_generic::Pose2d &ds = s.ds.getPose2d();
	ofs << time << " " << time + s.dt*0.5 << " " << s.dt << " " << s.c_step.v << " " << s.c_step.w << " " << mpac_generic::getDist(ds) << " " << s.ds.getSteeringAngle() << " " << s.cc_point.isValid() << " " << s.cc_point.c.v << " " << s.cc_point.c.w << " " << s.c_point.v << " " << s.c_point.w << " " << s.acc << " " << s.dir << " " << s.s_point.getSteeringAngle() << " " << s.ds.getSteeringAngle()/s.dt << " " << s.s_point.getPose2d()[0] << " " << ds[0]/s.dt << " " << s.ct_point << " " << (*this)[0].ct_point << std::endl;
	time += (*this)[i].dt;
      }
    ofs.close();
  }
  ...
  mpac_generic::Control getDeltaControl(size_t idx, bool fwd)  const
  {
    assert(idx < this->size());
    if (fwd) {
      return mpac_generic::Control((*this)[idx+1].c_point.v - (*this)[idx].c_point.v,
				    (*this)[idx+1].c_point.w - (*this)[idx].c_point.w);
    }
    else {
      return mpac_generic::Control((*this)[idx].c_point.v - (*this)[idx+1].c_point.v,
