...
#include <mpac_trajectory_processor/step_naive.h>
#include <mpac_trajectory_processor/trajectory_processor.h>
#include <cassert>
#include <mpac_generic/io.h>
#include <mpac_generic/path_utils.h>

class TrajectoryProcessorNaive : public TrajectoryProcessor, public mpac_generic::DeltaTInterface
{
 public:
 ...
 TrajectoryProcessorNaive() : _startIdx(0) {  }
  ...
  void computeDts()
	...
	mpac_generic::saveTrajectoryTextFile(_steps.convertNaiveStepsToTrajectory(), _params.debugPrefix + "naive_trajectoryXY.txt");
      }
    }
  
  mpac_generic::Trajectory getTrajectory() {
    computeDts();
	...
	mpac_generic::savePathTextFile(this->coordinationPathPoints(), _params.debugPrefix + "naive_trajectoryXY_coordination_times.txt");
      }
    }
    ...
    if (_params.debug) {
      ...
      mpac_generic::saveTrajectoryTextFile(output.convertNaiveStepsToTrajectory(), _params.debugPrefix + "naive_trajectory_fixed_dtXY.txt");
      mpac_generic::Path fwd_sim = mpac_generic::forwardSimulation(output.convertNaiveStepsToTrajectory(), _params.wheelBaseX, _params.timeStep);
      mpac_generic::savePathTextFile(fwd_sim, _params.debugPrefix + "naive_trajectory_fixed_dt_fwd_simXY.txt");
    }
  ...
  TrajectoryStepNaiveVec createNaiveTrajectoryFixedDt(TrajectoryStepNaiveVec &steps, double dt)
    {
      ...
      double time = 0;
      //      double v_prev = 0.;
      mpac_generic::Control control_step;
  ...
  void addPathInterface(const mpac_generic::PathInterface &path)
  {
    if (!mpac_generic::validPathForTrajectoryProcessing(path)) {
      std::cerr << "trajectory_processor : invalid path(!) - should never happen" << std::endl;
    }
    _path = mpac_generic::Path(path);
  }

  void addCoordinatedTimes(const mpac_generic::CoordinatedTimes &times) {
    _coordinatedTimes = times;
  }

  void addControlConstraintPoint(size_t stepIdx, const mpac_generic::Control &control) {
    _controlConstraintPoints.push_back(std::pair<size_t, mpac_generic::Control>(stepIdx, control));
  }

  void addControlConstraintPoints(const std::vector<std::pair<size_t, mpac_generic::Control> > &controlConstraintPoints) {
    for (size_t i = 0; i < controlConstraintPoints.size(); i++) {
      _controlConstraintPoints.push_back(controlConstraintPoints[i]);
    }
  }

  void addControlConstraintPointAsStart(size_t startIdx, const mpac_generic::Control &control) {
    _controlConstraintPoints.push_back(std::pair<size_t, mpac_generic::Control>(0, control));
    _startIdx = startIdx;
  }
  ...
  void setCoordinationTimeAtStartIdx(double factor) {
    ...
    double time_diff = mpac_generic::getTotalTimeBetween(*this, 0, next_ct_idx - _startIdx);
    _coordinatedTimes[_startIdx] = _coordinatedTimes[next_ct_idx] - time_diff*factor;
  }

 protected:
  mpac_generic::Path _path; // Original path
  mpac_generic::CoordinatedTimes _coordinatedTimes; // Coordinated times (if available -> must have same size as _path)

  TrajectoryStepNaiveVec _steps; // For the delta t interface.
  std::vector<std::pair<size_t, mpac_generic::Control> > _controlConstraintPoints; // To add constraints on control values on specific step points.
  ...
  mpac_generic::Path coordinationPathPoints() const {
  ...
  void setupSteps(TrajectoryStepNaiveVec &steps)
  {
    ...
    for (size_t i = _startIdx; i < _path.sizePath(); i++)
      {
	TrajectoryStepNaive s;
	s.s_point = mpac_generic::State2d(_path, i);
	if (use_ct)
	...
	double dist = mpac_generic::getDistBetween(steps.back().s_point.getPose2d(), _path.getPose2d(i));
	if (dist < _params.minDist)
	  {
	    std::cerr << "The states are not separated enough. This is a core requirement for this function. (use mpac_generic::minIncrementalDistancePath(), to fix your path), this path is saved as [traj_proc_fail.path]." << std::endl;
	    mpac_generic::savePathTextFile(_path, "traj_proc_fail.path");
	    assert(false);
	  }
	else
	  {
    ...
    steps.back().ds = mpac_generic::State2d(mpac_generic::Pose2d(0.,0.,0.), 0.);
    steps.back().dt = 0.001; // Avoid having any zero times.
    steps.back().c_step = mpac_generic::Control(0., 0.);
    steps.back().acc = 0.;
  ...
  void assignDs(TrajectoryStepNaiveVec &steps)
  {
    for (size_t i = 0; i < steps.size()-1; i++)
      {
	steps[i].ds = mpac_generic::subState2d(steps[i].s_point, steps[i+1].s_point);
      }
  }
  ...
  void assignDir(TrajectoryStepNaiveVec &steps) 
  {
	...
	double dir  = mpac_generic::getDirectionIncr(steps[i].ds.getPose2d());
	steps[i].dir = dir;
      }
  ...
  bool addAccelerationConstraint(TrajectoryStepNaiveVec &steps, int idx, bool fwd)
  {
    ...
    mpac_generic::Control dc = steps.getDeltaControl(step_idx, fwd);
 ...
 int getPrevPointIdx(const TrajectoryStepNaiveVec &steps, double time) const
  ...
  bool interpolateStatePoint(const TrajectoryStepNaiveVec &steps, const double &time, mpac_generic::State2d &state, double &usedTime) const
  {
	...
	//	double dir_dd = mpac_generic::getDist(steps[prev_idx].ds.getPose2d()) * mpac_generic::getDirectionIncr(steps[prev_idx].ds.getPose2d());
    ...
    //    assert(factor >= 0.);
    //    assert(factor < 1.);
    state = mpac_generic::addState2d(steps[prev_idx].s_point, steps[prev_idx].ds.scale(factor)); 
    return ret;
  }

  mpac_generic::State2d interpolateStatePoint(const TrajectoryStepNaiveVec &steps, const double &time)
  {
  ...
  bool interpolateControlStep(const TrajectoryStepNaiveVec &steps, const double &start_time, const double &stop_time, mpac_generic::Control &controlStep, double &usedDt) const
  {
    double start_used_time, stop_used_time;
    mpac_generic::State2d start, stop;
    ...
    //    controlStep = mpac_generic::Control(mpac_generic::getDist(ds.getPose2d()) / usedDt * , ds.getSteeringAngle() / usedDt);
  
    controlStep = start.getControlStep(stop, usedDt);

    // Debug
    //    mpac_generic::State2d ds = mpac_generic::subState2d(start, stop); // Fine, start and stop is points
    //    std::cout << "start time : " << start_time << " ds : " << mpac_generic::getDist(ds.getPose2d()) << std::endl;
  ...
  mpac_generic::Trajectory convertNaiveStepsToTrajectory(const TrajectoryStepNaiveVec &steps) const
    {
